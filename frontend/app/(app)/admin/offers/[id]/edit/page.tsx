...
export default function EditOfferPage() {
      ...
      try {
        ...
        if (o.valid_until) {
          const days = Math.round(
            (new Date(o.valid_until).getTime() - Date.now()) / 86400000,
          );
        ...
        if (calcJson) {
          try {
            ...
            if (calc.editingOfferId === offerId) {
              ...
              const components = (calc.result?.tech_view || []).map(
                (tv: any) => ({
              ...
              const updatedVariant: VariantDraft = {
                ...
                dbId: calc.editingVariantId
                  ? Number(calc.editingVariantId)
                  : undefined,
              ...
              if (calc.editingVariantId) {
                ...
                const idx = existingVariants.findIndex(
                  (v) => String(v.dbId) === String(calc.editingVariantId),
                );
                if (idx >= 0) {
                  updatedVariant.name = existingVariants[idx].name;
                  updatedVariant.isRecommended =
                    existingVariants[idx].isRecommended;
                ...
                if (!o.title && !calc.title && calc.templateName) {
                  setTitle(
                    `${calc.templateName} ${calc.width}×${calc.height} cm`,
                  );
                }
              }
            }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header */}
      <Header
        title={`Edycja oferty #${String(offer?.id || offerId).padStart(3, "0")}`}
        subtitle={
          hasNewCalc
            ? "✓ Wczytano nową kalkulację z kalkulatora"
            : "Edycja istniejącej oferty"
        }
        backHref={`/admin/offers/${offerId}`}
        actions={
          <>
            <button
            ...
            <button
              ...
              disabled={
                saving || variants.length === 0 || !selectedClient?.email
              }
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        {hasNewCalc && (
          <div className="bg-blue-50 border border-blue-200 text-blue-800 px-4 py-3 rounded-lg flex items-center gap-3">
            ...
            <p className="text-sm">
              Wariant oferty został zastąpiony nową kalkulacją z kalkulatora.
              Sprawdź szczegóły i zapisz.
            </p>
          </div>
        )}
        ...
        <div className="bg-white rounded-xl shadow-sm border p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">
            Warianty oferty ({variants.length})
          </h2>
          <div className="space-y-4">
            {variants.map((v) => (
              <div
                key={v.id}
                className={`rounded-xl border-2 p-5 ${v.isRecommended ? "border-emerald-300 bg-emerald-50/30" : "border-gray-200"}`}
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    {!v.isRecommended && (
                      <button
                        onClick={() => toggleRecommended(v.id)}
                          ...
                          <td className="px-4 py-2 text-center">
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                nv[vi].components[ci].visible_to_client =
                                  e.target.checked;
                                setVariants(nv);
                              }}
                ...
                <div className="mt-3 flex justify-between items-center pt-3 border-t">
                  ...
                  <div className="text-right">
                    <p className="text-lg font-bold text-gray-900">
                      {formatCurrency(v.totalPriceNet)}{" "}
                      <span className="text-sm font-normal text-gray-500">
                        netto
                      </span>
                    </p>
                    <p className="text-sm text-gray-500">
                      {formatCurrency(v.totalPriceGross)} brutto
                    </p>
                  </div>
                </div>
              </div>
