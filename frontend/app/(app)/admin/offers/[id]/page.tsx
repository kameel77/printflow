...
export default function OfferDetailPage() {
    ...
    const timeoutId = setTimeout(async () => {
      try {
        const res = await axios.get(
          `${API_URL}/clients?search=${encodeURIComponent(clientSearch)}`,
          {
            headers: getAuthHeaders(),
          },
        );
  ...
  const handleDuplicate = async () => {
    try {
      const res = await axios.post(
        `${API_URL}/offers/${offerId}/duplicate`,
        {},
        { headers: getAuthHeaders() },
      );
  ...
  const handleSaveClient = async () => {
    ...
    try {
      await axios.patch(
        `${API_URL}/clients/${offer.client.id}`,
        editingClientData,
        { headers: getAuthHeaders() },
      );
    ...
    if (offer) {
      ...
      if (offer.valid_until) {
        const days = Math.round(
          (new Date(offer.valid_until).getTime() - Date.now()) / 86400000,
        );
  ...
  const handleSaveOfferDetails = async () => {
    ...
    try {
      ...
      validUntil.setDate(
        validUntil.getDate() + parseInt(editValidDays || "14"),
      );
  ...
  const handleAddGlobalAdjustment = async () => {
    ...
    try {
      ...
      const currentTotalNet = offer.variants.reduce(
        (sum, v) => sum + Number(v.total_price_net),
        0,
      );
      const amountNet =
        adjustmentType === "amount" ? val : currentTotalNet * (val / 100);
      ...
      if (editingAdjustmentId) {
        ...
        await axios.delete(
          `${API_URL}/offers/${offerId}/variants/${editingAdjustmentId}`,
          {
            headers: getAuthHeaders(),
          },
        );
  ...
  const handleEditVariant = (variant: any) => {
    ...
    const editData = {
      ...
      width: variant.width_cm
        ? String(Number(variant.width_cm))
        : snap.width || "",
      height: variant.height_cm
        ? String(Number(variant.height_cm))
        : snap.height || "",
  ...
  const totalNet = variantsWithPrice.reduce(
    (sum, v) => sum + Number(v.total_price_net),
    0,
  );
  const totalGross = variantsWithPrice.reduce(
    (sum, v) => sum + Number(v.total_price_gross),
    0,
  );
  const totalCogs = variantsWithPrice.reduce((sum, v) => {
    ...
    if (snap.result?.total_cost_cogs !== undefined) {
      ...
      const marginPct =
        snap.finalMarginPercentage ?? snap.result?.margin_percentage ?? 0;
  ...
  const marginPct =
    totalCogs > 0 ? (marginVal / totalCogs) * 100 : totalNet > 0 ? 100 : 0;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <>
            {offer.status === "DRAFT" && (
              <button
                ...
                className="px-4 py-2 text-sm font-medium text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 transition-colors inline-flex items-center gap-2"
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
                ...
                <div className="flex gap-2">
                  ...
                  {offer.status === "DRAFT" && (
                    <button
                      ...
                      className="text-xs text-blue-600 hover:text-blue-700 font-medium"
                    >
                      {offer.client ? "Zmień klienta" : "Przypisz klienta"}
                    </button>
                  )}
                </div>
              )}
              ...
              <div className="space-y-3">
                          ...
                          <li key={client.id}>
                            ...
                            >
                              ...
                              <div className="text-xs text-gray-500 flex gap-2">
                                ...
                                {client.company_nip && (
                                  <span>NIP: {client.company_nip}</span>
                                )}
                              </div>
                            </button>
                          </li>
                ...
                <div className="flex gap-2 pt-2">
                  <button
                    ...
                    className="px-3 py-1.5 text-xs text-gray-600 border border-gray-300 rounded-md hover:bg-gray-50 transition-colors w-full"
                  >
                    Zakończ zmianę / Anuluj
                  </button>
                </div>
              </div>
            ) : offer.client ? (
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full text-sm border border-gray-300 rounded-md p-1.5 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none"
                    />
                  </div>
                  <div>
                    <label className="block text-xs text-gray-500 mb-1">
                    ...
                    <input
                      type="text"
                      value={editingClientData?.company_name}
                      onChange={(e) =>
                        setEditingClientData({
                          ...editingClientData,
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={editingClientData?.company_nip}
                      onChange={(e) =>
                        setEditingClientData({
                          ...editingClientData,
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div className="md:col-span-2">
                      ...
                      <input
                        type="text"
                        value={editingClientData.company_street || ""}
                        onChange={(e) =>
                          setEditingClientData({
                            ...editingClientData,
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={editingClientData.company_postal_code || ""}
                        onChange={(e) =>
                          setEditingClientData({
                            ...editingClientData,
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={editingClientData.company_city || ""}
                        onChange={(e) =>
                          setEditingClientData({
                            ...editingClientData,
                  ...
                  <div className="flex gap-2 pt-2">
                    ...
                    >
                      {savingClient ? "Zapisywanie..." : "Zapisz"}
                    </button>
                    <button
                ...
                <div>
                  ...
                  {offer.client.company_name && (
                    <p className="text-sm text-gray-500 mt-1">
                      {offer.client.company_name}{" "}
                      {offer.client.company_nip
                        ? `(NIP: ${offer.client.company_nip})`
                        : ""}
                    </p>
                  )}
                  {(offer.client.company_street ||
                    offer.client.company_postal_code ||
                    offer.client.company_city ||
                    offer.client.company_address) && (
                    <div className="mt-2">
          ...
          <div className="bg-white rounded-xl shadow-sm border p-5">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-gray-900">
                  {offer.viewed_at ? formatDate(offer.viewed_at) : "—"}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Odpowiedź:</span>
                <span className="text-gray-900">
                  {offer.responded_at ? formatDate(offer.responded_at) : "—"}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border p-5">
            <div className="flex justify-between items-center mb-3">
              ...
              {offer.status === "DRAFT" && !isEditingOfferDetails && (
                <button
                  onClick={handleEditOfferDetailsClick}
                  className="text-xs text-blue-600 hover:text-blue-700 font-medium"
                >
                  Edytuj szczegóły
                </button>
            ...
            {isEditingOfferDetails ? (
              <div className="space-y-3">
                ...
                <div className="flex gap-2 pt-2">
                  <button
                    ...
                    disabled={savingOfferDetails}
                    className="px-3 py-1.5 text-xs bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors w-full"
                  >
                    {savingOfferDetails ? "Zapisywanie..." : "Zapisz zamiany"}
                  </button>
                  <button
                    onClick={() => setIsEditingOfferDetails(false)}
                    className="px-3 py-1.5 text-xs text-gray-600 border border-gray-300 rounded-md hover:bg-gray-50 transition-colors w-full"
                  >
                    Gotowe / Anuluj
                  </button>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-900">
                    {offer.user
                      ? offer.user.full_name || offer.user.email
                      : "Brak danych"}
                  </span>
                </div>
                {offer.internal_note && (
        ...
        <div className="bg-white rounded-xl shadow-sm border p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900">
              Kalkulacje / Pozycje{" "}
              {offer.variants.length > 0 ? `(${offer.variants.length})` : ""}
            </h3>
            {offer.status === "DRAFT" && (
              <div className="flex gap-2">
                <button
          ...
          {isAddingAdjustment && (
            <div className="mb-6 p-4 border border-purple-100 bg-purple-50/50 rounded-xl">
              <h4 className="text-sm font-medium text-purple-900 mb-3">
                {editingAdjustmentId
                  ? "Edytuj korektę"
                  : "Dodaj korektę do zamówienia"}
              </h4>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
                <div className="md:col-span-2">
                  <label className="block text-xs font-medium text-gray-700 mb-1">
                    Nazwa (np. Rabat stałego klienta)
                  </label>
                  <input
                    type="text"
              ...
              <div className="flex gap-2 mt-4 justify-end">
                ...
                <button
                  ...
                  disabled={savingAdjustment || !adjustmentValue}
          ...
          {offer.variants.length === 0 ? (
            <div className="text-center py-6 text-gray-500 bg-gray-50 rounded-lg border border-dashed">
              Brak przeliczonych pozycji w ofercie. Kliknij "Dodaj nową
              kalkulację", aby rozpocząć.
            </div>
          ) : (
            <>
                ...
                {offer.variants
                  ...
                  .map((v, idx) => {
                    ...
                    return (
                      <div
                        key={v.id}
                        className={`rounded-xl border-2 p-5 ${isAccepted ? "border-green-300 bg-green-50/30" : v.is_recommended ? "border-emerald-200 bg-emerald-50/20" : "border-gray-200"}`}
                      >
                          ...
                          <div>
                            ...
                            <div className="flex gap-4 text-sm text-gray-600 mt-1">
                              {v.width_cm && v.height_cm && (
                                <span>
                                  Wymiary: {Number(v.width_cm)}×
                                  {Number(v.height_cm)} cm
                                </span>
                              )}
                              {v.quantity && (
                            ...
                            offer.client?.company_nip ? (
                              <>
                                <div className="flex items-center gap-3 w-full justify-end">
                                  ...
                                  {offer.status === "DRAFT" &&
                                    ((v as any).calculation_snapshot
                                      ?.is_global_adjustment ? (
                                      <button
                          ...
                          <table className="min-w-full divide-y divide-gray-200 text-sm">
                              ...
                              <tr>
                                ...
                                <th className="font-semibold text-gray-500 py-3 1/6 text-center w-16 border-b border-gray-100 pr-2">
                                  <span
                                    ...
                                    title="Czy pozycja jest widoczna dla klienta na kalkulacji"
                                  >
                                    <svg
                                      className="w-4 h-4"
                                      fill="none"
                            ...
                            <tbody className="divide-y divide-gray-100">
                              {v.components.map((c) => (
                                <tr
                                  key={c.id}
                                  className={`${!c.visible_to_client ? "opacity-50" : ""}`}
                                >
                                  ...
                                  <td className="px-4 py-2">
                                    <span
                                      className={`px-2 py-0.5 rounded text-xs font-medium ${c.type === "MATERIAL" ? "bg-purple-100 text-purple-700" : c.type === "ADJUSTMENT" ? "bg-green-100 text-green-700" : "bg-orange-100 text-orange-700"}`}
                                    >
                                      {c.type === "MATERIAL"
                                        ? "Materiał"
                                        : c.type === "ADJUSTMENT"
                ...
                <div className="mt-6 p-5 bg-blue-50/50 rounded-xl border border-blue-100 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                  <div className="flex-1">
                    <h4 className="font-medium text-gray-700 text-lg">
                      Podsumowanie całej oferty:
                    </h4>
                    <div className="mt-1 flex items-center gap-3">
                      <div className="px-3 py-1 bg-white border border-green-200 rounded-lg shadow-sm">
                        <span className="text-xs text-green-700 font-medium tracking-wide uppercase">
                          Zysk (Marża)
                        </span>
                    ...
                    <div className="flex items-center gap-3">
                      <span
                        className={`w-2 h-2 rounded-full ${
                          evt.event_type === "ACCEPTED"
                            ? "bg-green-500"
                            : evt.event_type === "REJECTED"
                              ? "bg-red-500"
                              : evt.event_type === "LINK_CLICKED"
                                ? "bg-blue-500"
                                : "bg-gray-400"
                        }`}
                      />
        ...
        <div className="bg-white rounded-xl shadow-sm border p-5">
          ...
          <div className="flex items-center gap-2">
            <code className="flex-1 px-3 py-2 bg-gray-100 rounded-lg text-sm text-gray-600 truncate">
              {publicLink}
            </code>
            <button
              onClick={copyLink}
      ...
      <SendOfferModal
        ...
        onSend={handleSend}
        clientName={
          offer.client?.name || offer.client?.company_name || undefined
        }
      />
    </div>
  );
