            ...
            <div>
              ...
              {offer.client_name && (
                <p className="text-gray-600 mt-1">
                  Przygotowana dla: <strong>{offer.client_name}</strong>
                </p>
              )}
            </div>
            {offer.valid_until && (
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="space-y-4">
          {offer.variants.map((v, idx) => {
            const visibleComponents = v.components.filter(
              (c) => c.visible_to_client,
            );
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-1 text-sm text-gray-500">
                        {v.width_cm && v.height_cm && (
                          <span>
                            {Number(v.width_cm)}×{Number(v.height_cm)} cm
                          </span>
                        )}
                    ...
                    {offer.company_name ? (
                      <>
                        <p className="text-2xl font-bold text-gray-900">
                          {formatCurrency(v.total_price_net)}{" "}
                          <span className="text-sm font-normal text-gray-500">
                            netto
                          </span>
                        </p>
                        <p className="text-sm text-gray-500">
                          {formatCurrency(v.total_price_gross)} brutto
                        </p>
                      </>
                    ) : (
                      <>
                        <p className="text-2xl font-bold text-gray-900">
                          {formatCurrency(v.total_price_gross)}{" "}
                          <span className="text-sm font-normal text-gray-500">
                            brutto
                          </span>
                        </p>
                        <p className="text-sm text-gray-500">
                          {formatCurrency(v.total_price_net)} netto
                        </p>
                      </>
                    )}
                  </div>
          ...
          <div className="bg-blue-50/50 rounded-2xl shadow-sm border border-blue-100 p-6 flex flex-col md:flex-row items-start md:items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Suma wszystkich pozycji ({offer.variants.length}) w ofercie
              </p>
            </div>
        ...
        {(offer.company_email || offer.company_phone) && (
          <div className="text-center py-6 text-sm text-gray-500">
            ...
            <div className="flex items-center justify-center gap-4 mt-2">
              {offer.company_email && (
                <a
                  href={`mailto:${offer.company_email}`}
